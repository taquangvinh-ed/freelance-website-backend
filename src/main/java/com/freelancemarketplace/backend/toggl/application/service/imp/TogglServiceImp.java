...
import com.freelancemarketplace.backend.toggl.dto.TogglUserResponseDTO;
...
import com.freelancemarketplace.backend.contract.infrastructure.repository.TimeLogRepository;
...
public class TogglServiceImp implements TogglService {
    ...
    public String getOrCreateFreelancerUser(Long freelancerId, String freelancerName, String email) {
        ...
        if (orgId == null || orgId.isBlank()) {
            throw new ApiException(HttpStatus.INTERNAL_SERVER_ERROR, ErrorCode.CONFIGURATION_ERROR,
                    "Toggl organization ID is not configured");
        }
        ...
        try {
            ...
            throw new ApiException(HttpStatus.BAD_GATEWAY, ErrorCode.EXTERNAL_SERVICE_ERROR,
                    "Toggl create user failed: " + response.getStatusCode());
        } catch (HttpClientErrorException e) {
            throw new ApiException(HttpStatus.BAD_GATEWAY, ErrorCode.EXTERNAL_SERVICE_ERROR,
                    "Toggl API Error (Create User): " + e.getResponseBodyAsString(), e);
        }
    }
        ...
        try {
            ...
            if (response.getStatusCode().is2xxSuccessful() && response.getBody() != null) {
                ...
                if (projectIdObject instanceof Number) {
                    ...
                    throw new ApiException(HttpStatus.BAD_GATEWAY, ErrorCode.EXTERNAL_SERVICE_ERROR,
                            "Toggl project creation failed: ID is missing or malformed.");
                }
            }
            throw new ApiException(HttpStatus.BAD_GATEWAY, ErrorCode.EXTERNAL_SERVICE_ERROR,
    ...
    log.info("toggl id: {}",togglProjectId);
        if (timeLogRepository.existsByFreelancerAndStatus(freelancer, TimeLogStatus.ACTIVE)) {
            throw new ApiException(HttpStatus.BAD_REQUEST, ErrorCode.INVALID_OPERATION,
                    "Freelancer already has an active timer running in contract " + contractId);
        }
        ...
        try {
            ...
            if (response.getStatusCode().is2xxSuccessful() && response.getBody() != null) {
                ...
                throw new ApiException(HttpStatus.BAD_GATEWAY, ErrorCode.EXTERNAL_SERVICE_ERROR,
                        "Toggl returned unexpected status: " + response.getStatusCode());
            }
        } catch (HttpClientErrorException e) {
            // Xử lý lỗi API client (ví dụ: 401 Unauthorized, 400 Bad Request)
            throw new ApiException(HttpStatus.BAD_GATEWAY, ErrorCode.EXTERNAL_SERVICE_ERROR,
                    "Toggl API Error (Start): " + e.getResponseBodyAsString(), e);
        }
    }
    ...
    public TogglTimeEntryResponseDTO stopTimeEntry(Long freelancerId, Long contractId) { // Đã đổi kiểu trả về
        ...
        Optional<TimeLog> timelog = Optional.ofNullable(activeTimeLog.stream().filter(log -> log.getContract().getContractId().equals(contractId)).findFirst()
                .orElseThrow(() -> new ApiException(HttpStatus.BAD_REQUEST, ErrorCode.INVALID_OPERATION,
        ...
        try {
            ...
            if (response.getStatusCode().is2xxSuccessful() && response.getBody() != null) {
                ...
                throw new ApiException(HttpStatus.BAD_GATEWAY, ErrorCode.EXTERNAL_SERVICE_ERROR,
                        "Toggl returned unexpected status (Stop): " + response.getStatusCode());
            }
        } catch (HttpClientErrorException e) {
            // Xử lý lỗi API client
            throw new ApiException(HttpStatus.BAD_GATEWAY, ErrorCode.EXTERNAL_SERVICE_ERROR,
                    "Toggl API Error (Stop): " + e.getResponseBodyAsString(), e);
        }
    }
