...
import com.freelancemarketplace.backend.payment.domain.enums.PaymentTypes;
    ...
    public String createStripeConnectAccount(String email, String countryCode, Long freelancerId) throws StripeException {
        try {
            ...
            throw new ApiException(HttpStatus.BAD_GATEWAY, ErrorCode.EXTERNAL_SERVICE_ERROR,
                    "Error create stripe connect account: " + e.getMessage(), e);
        }
    }
    ...
    public PaymentModel createHourlyPaymentFromWeeklyReport(ContractModel contract, WeeklyReportModel weeklyReport) {
        if (contract == null || weeklyReport == null) {
            throw new ApiException(HttpStatus.BAD_REQUEST, ErrorCode.INVALID_REQUEST,
                    "Contract and weeklyReport must not be null");
        }

        if (weeklyReport.getWeeklyReportId() == null) {
            throw new ApiException(HttpStatus.BAD_REQUEST, ErrorCode.INVALID_REQUEST,
                    "Weekly report must be persisted before creating payment");
        }
