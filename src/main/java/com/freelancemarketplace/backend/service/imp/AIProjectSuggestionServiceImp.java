...
public class AIProjectSuggestionServiceImp implements AIProjectSuggestionService {
    ...
    public AiSuggestProjectResponseDTO suggestProject(Long userId, AiSuggestProjectRequest request) {
        ...
        if (anthropicApiKey == null || anthropicApiKey.isBlank()) {
            throw new IllegalArgumentException("ANTHROPIC_API_KEY is missing. Please set it in environment variables.");
        }
    ...
    private void validateRateLimit(Long userId) {
        ...
        synchronized (timeline) {
            ...
            if (timeline.size() >= MAX_REQUESTS_PER_HOUR) {
                throw new IllegalStateException("Rate limit exceeded: max 10 AI requests per hour.");
            }
    ...
    private String callClaude(String systemPrompt, String userPrompt, String requestId) throws IOException, InterruptedException {
        ...
        if (response == null) {
            throw new RuntimeException("Claude API did not return a response");
        }

        if (response.statusCode() == 401) {
            throw new RuntimeException("Claude API 401. Check ANTHROPIC_API_KEY and project permissions.");
        }

        if (response.statusCode() >= 400) {
            throw new RuntimeException("Claude API call failed: HTTP " + response.statusCode());
        }
        ...
        if (text == null || text.isBlank()) {
            throw new RuntimeException("Claude API returned blank text content");
        }
